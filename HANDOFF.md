...
## 2026-09-08 16:07 — "한잔의 커피" Works 항목 신규 추가, 영상 임베드 조건 버그 수정, 커밋·푸시 완료

**지금 상태**: 15:22 항목 이후 이어서 진행, 4개 커밋 완료·푸시(`c880992`, `40b3f19`, `003b90a`, `547c6a7`).
1. 사용자가 `~/dev/29sfilms_coffee/`(별도 로컬 프로젝트, 29초영화제 출품용 생성형 AI 단편영화)에 완성된 작업이 있다고 해서 확인 후 uzun-choi Works에 새로 추가함. 완성 mp4(`29초_영화제_커피_최우준.mp4`, 1920×1080/29.8초)에서 `ffmpeg`로 8프레임 캡쳐(균등 간격, 8번째는 화이트아웃이라 제외) → 7장을 새 작업 `content/works/2026-17-hanjanui-coffee/`의 미디어로 사용. 슬러그 번호 컨벤션(번호 높을수록 최신 → `order` 낮음)에 따라 `order: 1`로 넣고 기존 26개 작업 `order`를 전부 +1(`node -e` 스크립트로 일괄 처리 후 `credit` 배열 압축 포맷 깨짐을 정규식으로 재압축 — [[uzun-choi 모바일 반응형 빌드]]에 기록된 과거 `JSON.stringify(obj, null, 2)` 함정이 이번에도 재발했었음). `title.en`은 아직 미확정이라 국문과 동일하게 채워둠(다른 미번역 제목 작업들 관례). `credit` 필드는 사용자가 전부 직접 작업했다고 해서 생략. 홈 히어로엔 미노출, Works 그리드에만. 커버는 처음 컵에 따르는 컷 → 사용자 요청으로 시계 클로즈업 컷으로 교체.
2. 작업하다가 `WorkDetail.tsx`의 구조적 버그 발견: 유튜브 임베드(`videoUrl`)가 스택 갤러리 섹션(`showStacked = total > 8`) 안에 중첩돼 있어서 미디어가 8장 안 넘으면 영상 설정돼 있어도 아예 안 뜨는 조건이었음 — 이번 작업이 스틸컷 7장이라 정확히 이 케이스였음. `{(showStacked || work.videoUrl) && (...)}`로 수정해서 영상이 스틸컷 개수와 무관하게 항상 뜨도록 고침(다른 영상 작업에도 적용되는 일반 수정).
3. 설명(`content.ko.md`/`content.en.md`)에 "생성형 AI 영상을 활용하여 완성했다"는 취지로 작성, 엔딩 카피("나를 위한 완벽한 한잔")를 사용자 요청으로 양쪽 따옴표 넣어 인용.
4. 옵시디언도 업데이트: [[uzun-choi 한잔의 커피 작업 추가]] 신규 노트, [[uzun-choi 웹 포트폴리오]] 허브에 링크 추가, 원작 프로젝트 쪽 [[집착의 커피 (29초영화제 출품작)]]/[[커피 29초영화제]]에도 YouTube 링크(`https://youtu.be/8GvLZWUlE_I`)와 포트폴리오 게시 사실 반영.

**남은 일**: 영문 제목 확정되면 `content/works/2026-17-hanjanui-coffee/meta.json`의 `title.en` 교체 필요(현재 국문과 동일한 임시값). 그 외 이번 세션 요청 항목은 전부 완료.

**하면 안 되는 일**: 새 작업을 Works 맨 앞(`order: 1`)에 넣을 땐 반드시 기존 전체 작업의 `order`를 밀어줘야 함(슬러그 번호=최신도 컨벤션과 어긋나면 그리드 정렬이 깨짐). `meta.json`을 스크립트로 일괄 수정할 때 `JSON.stringify(obj, null, 2)`(또는 Python `json.dump(indent=2)`)를 그대로 쓰면 `credit` 배열의 압축 한 줄 포맷이 깨짐 — 커밋 전 `git diff`로 의도치 않은 재포맷 없는지 항상 확인할 것.

**검증 방법**: `git log --oneline -5`에 `547c6a7`이 최신, `git status`는 이 항목이 다루는 파일들에 한해 clean(아래 참고 — 무관한 uncommitted 상태 있음). `http://localhost:3100/works/2026-17-hanjanui-coffee`에서 유튜브 임베드(`youtube.com/embed/8GvLZWUlE_I`)가 뜨는지, `http://localhost:3100/works` 그리드 맨 앞에 시계 클로즈업 썸네일로 노출되는지 확인.

**참고**: 세션 종료 시점에 이 작업과 **무관한** uncommitted 상태가 저장소에 있었음 — `package.json`/`package-lock.json`에 `playwright` 의존성 추가, `scripts/site-tour.mjs`(Playwright로 사이트 전체를 데스크톱/모바일 뷰포트로 녹화·스크린샷하는 원샷 스크립트), `tour-output/`(스크린샷·zip·영상 결과물, 수십MB). 이번 세션에서 만든 게 아니고 이 세션 내내 존재를 인지 못 하다가 마지막에 발견함 — 다음 세션에서 사용자에게 의도한 작업인지 확인 후 커밋하거나 정리할 것, 임의로 지우지 말 것. 새 작업 폴더는 `content/works/2026-17-hanjanui-coffee/`, 관련 코드는 `WorkDetail.tsx`의 `showStacked || work.videoUrl` 조건.

## 2026-09-08 15:22 — 무비랜드 미디어 파일명 재정렬, 히어로/라이트박스 이미지 프리로드 추가, 커밋·푸시 완료

**지금 상태**: 14:42 항목 작업(커밋 `a3dfb35`, `93f8c5c`) 이후, 같은 세션에서 이어서 두 가지 추가 처리, 각각 커밋·푸시 완료(`c6cf4f1`, `c00fb2e`).
1. Movie Land(`2026-09-movie-land`) 미디어 파일명이 `NN_SHOP-M.jpg` 형태인데 `NN`(정렬 순번)과 `SHOP-M`의 `M`이 안 맞아서(예: `03_SHOP-11.jpg`, `17_SHOP-10.jpg`) 히어로/갤러리 표시 순서가 뒤죽박죽이었음(디렉터리 파일명 정렬 기반이라). `-` 뒤 숫자(`M`) 오름차순으로 `NN`을 재부여(`SHOP-1`→`SHOP-18` 순), `shop-add-1`은 추가컷이라 맨 뒤(19) 유지. `media/`와 이미 생성해둔 `media/detail/`(직전 커밋에서 만든 리사이즈본) 둘 다 새 이름으로 리네임/재생성해서 두 폴더가 항상 짝이 맞도록 함. 표지(`01_SHOP-1.jpg`)는 원래도 1번이라 변경 없음.
2. "상세페이지 스크롤버튼(히어로 캐러셀 ScrollNav) 누를 때 로딩이 느리다"는 피드백 → 원인은 클릭 시점에야 다음 슬라이드 이미지를 요청하는 구조(사전 로드 없음)였음. `WorkDetail.tsx`에 프리로드 `useEffect` 2개 추가 — 히어로 인덱스가 바뀔 때마다 앞뒤 슬라이드를, 라이트박스 인덱스가 바뀔 때마다 앞뒤 이미지를 `new window.Image().src = ...`로 백그라운드 캐시 워밍. 사용자가 로컬에서 확인 후 "많이 좋아졌다"고 확인함.

**남은 일**: 없음. 이번 세션에 요청받은 항목(제주 파일명 오류 수정, 상세페이지 로딩 속도 개선, 무비랜드 파일명 정렬) 전부 반영·확인·커밋·푸시 완료.

**하면 안 되는 일**: 콘텐츠 폴더(`content/works/*/media/`)의 원본 사진 파일명이 사용자에 의해 세션 중간에 예고 없이 바뀔 수 있음(이번 세션에 제주·무비랜드 둘 다 그랬음) — `meta.json`의 `cover`/`heroMedia`/`stackedMedia` 등 명시적 파일명 참조와 `media/detail/`의 리사이즈본은 원본과 파일명이 정확히 일치해야 하므로, 원본 리네임 작업을 할 땐 항상 `media/detail/`도 같이 지우고 `node scripts/generate-detail-images.mjs`로 재생성할 것(안 그러면 새 파일명엔 리사이즈본이 없어서 라우트 폴백으로 원본이 나가긴 하지만, 옛 이름의 고아 파일이 계속 남음).

## 2026-09-08 14:42 — 제주 사진 파일명 참조 깨짐 수정, 상세페이지 이미지 최적화(전 작업 대상) 완료

**지금 상태**: 커밋 예정(이 응답 직후 진행). 두 가지 작업:
1. `content/works/2026-14-jeju/media/`의 사진 파일명이 `01__.jpg`/`02__.jpg`/`03__-.jpg` → `01.jpg`/`02.jpg`/`03.jpg`로 바뀌어 있었는데, `meta.json`의 `cover`가 옛 파일명을 그대로 참조해서 표지/히어로 이미지가 깨져 있었음 → `cover`를 `media/01.jpg`로 수정, 옛 파일명 기준으로 만들어져 있던 그리드 썸네일(`thumb/01__.jpg`)도 지우고 새 파일명 기준(`thumb/01.jpg`)으로 재생성. 코드 전체에 옛 파일명 잔여 참조 없음 확인.
2. 상세페이지("상세페이지 로딩이 느리다"는 사용자 피드백) 원인 파악: `WorkDetail.tsx`의 히어로/스택/라이트박스 이미지가 `next/image`를 안 거치고 `<img src>`로 원본을 그대로 물고 있었고, 그걸 서빙하는 `app/works-media/[...path]/route.ts`도 리사이징 없이 원본 바이트를 그대로 반환 — 원본이 최대 70MB(제주), 20~34MB짜리가 수두룩(Movie Land 등)해서 전체 상세페이지가 다 느렸음(지난 세션에 그리드 썸네일만 최적화했고 상세페이지 원본은 손 안 댄 상태였음). `scripts/generate-detail-images.mjs` 신설(기존 `generate-thumbnails.mjs`와 동일 패턴, `sips` 사용, 새 의존성 없음) — 긴 변 2400px 초과 사진만 JPEG 품질 82로 리사이즈해 `media/detail/<파일명>`에 저장, 27개 작업 전체 대상 1회 실행(170장 생성, 이미 작은 50장은 스킵). `app/works-media/[...path]/route.ts`에 폴백 추가: `.../media/detail/x.jpg` 요청 시 그 파일이 없으면 원본 `.../media/x.jpg`로 자동 대체. `WorkDetail.tsx`는 히어로 이미지·스택 갤러리·라이트박스 기본(확대 전) 뷰만 `/detail/` 경로를 쓰도록 변경, 라이트박스에서 사용자가 "확대"를 누른 경우(`isZoomed`)만 원본을 그대로 서빙(의도적 — 확대는 원본 화질을 보고 싶어서 누르는 액션).

**남은 일**: 이번 응답 직후 커밋·푸시 진행. 이후 남은 일 없음 — 사용자가 로컬(`localhost:3100`)에서 이미 확인 완료.

**하면 안 되는 일**: `media/detail/`은 원본보다 작은 파일만 생성됨(2400px 이하 원본은 스킵) — 리사이즈본이 없는 파일에 대해 `/detail/` 경로로 직접 접근하면 라우트가 원본으로 폴백하니, 컴포넌트 쪽에서 파일별 존재 여부를 따로 체크할 필요 없음(그 로직을 다시 추가하려 하지 말 것, 라우트가 이미 처리함). 라이트박스 확대(zoom) 모드는 의도적으로 원본을 그대로 쓰는 것 — 용량 줄인다고 여기도 `/detail/`로 바꾸면 안 됨(사용자가 명시적으로 원본 화질 확인용으로 원한 동작).

**검증 방법**: `git log --oneline -3`에 이번 세션 커밋이 최신, `git status` clean 확인. `http://localhost:3100/works/2026-14-jeju`에서 이미지 정상 렌더링(표지/히어로/갤러리), `/works` 그리드에서도 제주 썸네일 정상 확인. 네트워크 탭에서 상세페이지 이미지 요청이 수백 KB~2-3MB대인지(원본 수십 MB가 아닌지), 라이트박스에서 "확대"를 눌렀을 때만 큰 원본이 로드되는지 확인.

**참고**: `scripts/generate-detail-images.mjs`(`TARGET_MAX_SIDE = 2400`, `JPEG_QUALITY = 82`), `app/works-media/[...path]/route.ts`의 `detail` 폴백 로직, `WorkDetail.tsx`의 `toDisplayUrl()`. 새 콘텐츠 추가 워크플로우 문서(`content/works/README.md`)에 아직 반영 안 함 — 새 작업 사진 넣을 때 `node scripts/generate-thumbnails.mjs`와 `node scripts/generate-detail-images.mjs` 둘 다 돌려야 함(이미 있는 파일은 스킵).

## 2026-09-08 14:05 — 모바일 헤더 높이·여백 축소, Works 그리드 간격 동기화, 커밋·푸시 완료

**지금 상태**: 세 커밋으로 나눠 커밋·푸시 완료(`3cc59c9`, `10587d7` — 이번 세션 작업, 그 전 `1a819ae`는 지난 세션), `git status` clean, `origin/main` 동기화됨. 모바일(unprefixed)만 변경, 데스크톱(`md:`)은 전부 그대로.
1. `Header.tsx`: 좌우 여백을 25→20px 순서로 조정하다 최종 **20px**로 확정, 헤더 높이는 110px→90px→**80px**로 단계적으로 축소(안쪽 박스 높이를 바깥과 동일하게 맞춰 상하 여백 0 유지), 이후 사용자가 헤더 자체를 화면 최상단에서 **5px 띄워달라**고 해서 `top-0` → `top-[1.28205vw] md:top-0`으로 변경.
2. 헤더 높이/위치가 바뀔 때마다 그걸 기준으로 계산돼 있던 다른 값들을 전부 같이 재계산함(안 그러면 겹치거나 빈 간격 생김): `WorksGrid.tsx`/`WorkDetail.tsx`의 본문 상단 `pt-`, `WorkDetail.tsx` 라이트박스 닫기 버튼의 `top-`/높이(주석에 "헤더와 같은 여백"이라 명시된 곳, 실측으로 헤더-버튼 세로 중심 일치 확인), `MenuOverlay.tsx` 메뉴 상단 `pt-`.
3. Works 그리드(모바일): 태그 페이저("ALL/PHOTO/...")가 헤더 높이 변경 이후 그리드와 관계가 깨져서 "많이 밀렸다"는 피드백 → 태그 박스 하단이 그리드 시작 지점과 정확히 맞물리는(flush) 원래 설계 관계를 복원하면서, 헤더-태그 간격은 20→10→**5px**로 사용자가 여러 번 조정 요청해서 확정. 그리드 좌우 여백 27→**25px**, 카드 간격 8.6→**7.5px**로 조정(태그 페이저 왼쪽 `pl-`도 그리드 왼쪽 끝과 맞춰 같이 변경).
4. 매 변경마다 Chrome 자동화(모바일 390px 뷰포트)로 헤더/태그/그리드/라이트박스/메뉴 오버레이 위치를 `getBoundingClientRect()` 실측으로 검증하며 진행 — 스크린샷만으론 안 보이는 px 단위 오차까지 확인함.

**남은 일**: 없음. 이번 세션에 요청받은 항목 전부 반영·검증·커밋·푸시 완료.

**하면 안 되는 일**: 헤더 높이(`h-[...]`, `Header.tsx`)나 위치(`top-[...]`)를 다시 바꿀 땐 반드시 [[uzun-choi 모바일 헤더 높이·여백 튜닝]]에 정리된 연쇄 의존 관계(본문 `pt-`, 라이트박스 닫기 버튼, 메뉴 오버레이 `pt-`, Works 태그 페이저 `top-`/그리드 `mt-`)를 전부 같이 재계산할 것 — 하나만 바꾸면 겹침/빈 간격이 생김. Works 태그 페이저의 `top-`과 그리드의 `mt-`는 "태그 박스 하단 = 그리드 시작 지점"이 불변식이므로, 헤더-태그 간격만 바꾸고 싶으면 `mt-`는 그 불변식을 유지하도록 같이 계산해야 함(직접 계산식은 위 참고 노트에 정리).

**검증 방법**: `git log --oneline -3`에 `10587d7`이 최신, `git status` clean 확인. `http://localhost:3100/`(또는 현재 실행 중인 포트)를 모바일 폭(390px)에서 확인 — 헤더가 화면 위에서 5px 떨어져 시작하는지, `/works`에서 "ALL" 텍스트가 헤더 아래 5px 지점에서 시작하고 그 아래 그리드 카드와 딱 이어지는지(간격 없이), 좌우 여백 25px·카드 간격 7.5px인지, 작업 상세 페이지 라이트박스 닫기 X가 헤더의 햄버거/X와 같은 높이에 있는지, 메뉴 오버레이도 겹치지 않는지 확인.

**참고**: 커밋 `3cc59c9`(헤더 높이·패딩·그리드 간격), `10587d7`(헤더 5px 상단 여백 전파). `Header.tsx`, `WorksGrid.tsx`(태그 페이저 `top-`/그리드 `mt-`), `WorkDetail.tsx`(라이트박스 닫기 버튼), `MenuOverlay.tsx`(`pt-`). 위키 노트 [[uzun-choi 모바일 헤더 높이·여백 튜닝]]에 정확한 계산식(px↔vw 변환, 불변식) 정리해둠.
